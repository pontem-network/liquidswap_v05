#[test_only]
module liquidswap_v05::router_tests {
    use std::signer;

    use aptos_framework::fungible_asset;
    use aptos_framework::primary_fungible_store;
    use aptos_framework::timestamp;

    use liquidswap_v05::curves::{Uncorrelated, Stable};
    use liquidswap_v05::liquidity_pool;
    use liquidswap_v05::router;
    use test_coin_admin::test_coins::{Self, USDT, BTC, USDC};
    use test_helpers::test_pool;

    const MAX_U64: u64 = 18446744073709551615;

    fun register_pool_with_liquidity(x_val: u64, y_val: u64): (signer, signer) {
        let (fa_admin, lp_owner) = test_pool::setup_fa_and_lp_owner();

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"BTC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        router::register_pool<BTC, USDT, Uncorrelated>(
            &lp_owner,
            fa_x_metadata,
            fa_y_metadata,
        );

        let lp_owner_addr = signer::address_of(&lp_owner);
        if (x_val != 0 && y_val != 0) {
            let btc_fa = test_coins::mint_fa(&fa_admin,  b"BTC", x_val);
            let usdt_fa = test_coins::mint_fa(&fa_admin,  b"USDT", y_val);
            let lp_fa =
                liquidity_pool::mint<BTC, USDT, Uncorrelated>(btc_fa, usdt_fa);
            primary_fungible_store::deposit(lp_owner_addr, lp_fa);
        };

        (fa_admin, lp_owner)
    }

    fun register_stable_pool_with_liquidity(x_val: u64, y_val: u64): (signer, signer) {
        let (fa_admin, lp_owner) = test_pool::setup_fa_and_lp_owner();

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"USDC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        router::register_pool<USDC, USDT, Stable>(
            &lp_owner,
            fa_x_metadata,
            fa_y_metadata,
        );

        let lp_owner_addr = signer::address_of(&lp_owner);
        if (x_val != 0 && y_val != 0) {
            let usdc_fa = test_coins::mint_fa(&fa_admin,  b"USDC", x_val);
            let usdt_fa = test_coins::mint_fa(&fa_admin,  b"USDT", y_val);
            let lp_fa =
                liquidity_pool::mint<USDC, USDT, Stable>(usdc_fa, usdt_fa);
            primary_fungible_store::deposit(lp_owner_addr, lp_fa);
        };

        (fa_admin, lp_owner)
    }

    #[test]
    #[expected_failure(abort_code = router::ERR_WRONG_FA_ORDER)]
    fun test_register_pool_wrong_metadata_ordering_should_fail() {
        let (_, lp_owner) = test_pool::setup_fa_and_lp_owner();

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"USDC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        router::register_pool<USDC, USDT, Stable>(
            &lp_owner,
            fa_y_metadata,
            fa_x_metadata,
        );
    }

    #[test]
    fun test_add_initial_liquidity() {
        let (fa_admin, lp_owner) = register_pool_with_liquidity(0, 0);

        let btc_fa = test_coins::mint_fa(&fa_admin,  b"BTC", 101);
        let usdt_fa = test_coins::mint_fa(&fa_admin,  b"USDT", 10100);

        let (coin_x, coin_y, lp_fa) =
            router::add_liquidity<BTC, USDT, Uncorrelated>(
                btc_fa,
                101,
                usdt_fa,
                10100,
            );

        assert!(fungible_asset::amount(&coin_x) == 0, 0);
        assert!(fungible_asset::amount(&coin_y) == 0, 1);
        // 1010 - 1000 = 10
        assert!(fungible_asset::amount(&lp_fa) == 10, 2);

        primary_fungible_store::deposit(signer::address_of(&lp_owner), coin_x);
        primary_fungible_store::deposit(signer::address_of(&lp_owner), coin_y);
        primary_fungible_store::deposit(signer::address_of(&lp_owner), lp_fa);
    }

    #[test]
    fun test_add_liquidity_to_pool() {
        let (fa_admin, lp_owner) = register_pool_with_liquidity(101, 10100);

        let btc_fa = test_coins::mint_fa(&fa_admin,  b"BTC", 101);
        let usdt_fa = test_coins::mint_fa(&fa_admin,  b"USDT", 9000);

        let (coin_x, coin_y, lp_fa) =
            router::add_liquidity<BTC, USDT, Uncorrelated>(
                btc_fa,
                10,
                usdt_fa,
                9000,
            );
        assert!(fungible_asset::amount(&coin_x) == 11, 0);
        assert!(fungible_asset::amount(&coin_y) == 0, 1);

        assert!(fungible_asset::amount(&lp_fa) == 900, 2);

        primary_fungible_store::deposit(signer::address_of(&lp_owner), coin_x);
        primary_fungible_store::deposit(signer::address_of(&lp_owner), coin_y);
        primary_fungible_store::deposit(signer::address_of(&lp_owner), lp_fa);
    }

    #[test]
    #[expected_failure(abort_code = router::ERR_WRONG_FA_ORDER)]
    fun test_cannot_add_liquidity_to_pool_in_reverse_order() {
        let (fa_admin, lp_owner) = register_pool_with_liquidity(101, 10100);

        let btc_fa = test_coins::mint_fa(&fa_admin,  b"BTC", 101);
        let usdt_fa = test_coins::mint_fa(&fa_admin,  b"USDT", 9000);

        let (coin_y, coin_x, lp_fa) =
            router::add_liquidity<USDT, BTC, Uncorrelated>(
                usdt_fa,
                9000,
                btc_fa,
                10,
            );

        primary_fungible_store::deposit(signer::address_of(&lp_owner), coin_x);
        primary_fungible_store::deposit(signer::address_of(&lp_owner), coin_y);
        primary_fungible_store::deposit(signer::address_of(&lp_owner), lp_fa);
    }

    #[test]
    #[expected_failure(abort_code = router::ERR_INSUFFICIENT_X_AMOUNT)]
    fun test_add_liquidity_to_fail_with_insufficient_x_coins() {
        let (fa_admin, lp_owner) = register_pool_with_liquidity(0, 0);

        let btc_fa = test_coins::mint_fa(&fa_admin,  b"BTC", 101);
        let usdt_fa = test_coins::mint_fa(&fa_admin,  b"USDT", 9000);

        let (coin_y, coin_x, lp_fa) =
            router::add_liquidity<BTC, USDT, Uncorrelated>(
                btc_fa,
                102,
                usdt_fa,
                9000,
            );

        primary_fungible_store::deposit(signer::address_of(&lp_owner), coin_x);
        primary_fungible_store::deposit(signer::address_of(&lp_owner), coin_y);
        primary_fungible_store::deposit(signer::address_of(&lp_owner), lp_fa);
    }

    #[test]
    #[expected_failure(abort_code = router::ERR_INSUFFICIENT_Y_AMOUNT)]
    fun test_add_liquidity_to_fail_with_insufficient_y_coins() {
        let (fa_admin, lp_owner) = register_pool_with_liquidity(0, 0);

        let btc_fa = test_coins::mint_fa(&fa_admin,  b"BTC", 101);
        let usdt_fa = test_coins::mint_fa(&fa_admin,  b"USDT", 9000);

        let (coin_y, coin_x, lp_fa) =
            router::add_liquidity<BTC, USDT, Uncorrelated>(
                btc_fa,
                101,
                usdt_fa,
                10000,
            );

        primary_fungible_store::deposit(signer::address_of(&lp_owner), coin_x);
        primary_fungible_store::deposit(signer::address_of(&lp_owner), coin_y);
        primary_fungible_store::deposit(signer::address_of(&lp_owner), lp_fa);
    }

    #[test]
    fun test_remove_liquidity() {
        let (_, lp_owner) = register_pool_with_liquidity(101, 10100);

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"BTC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        let lp_metadata =
            liquidity_pool::get_pool_lp_metadata<BTC, USDT, Uncorrelated>(fa_x_metadata, fa_y_metadata);
        let lp_fa_val = 10;
        let lp_fa_to_burn =
            primary_fungible_store::withdraw(&lp_owner, lp_metadata, lp_fa_val);

        let (x_out, y_out) = router::get_reserves_for_lp_coins<BTC, USDT, Uncorrelated>(
            lp_fa_val,
            fa_x_metadata,
            fa_y_metadata,
        );
        let (coin_x, coin_y) =
            router::remove_liquidity<BTC, USDT, Uncorrelated>(
                lp_fa_to_burn,
                x_out,
                y_out,
                fa_x_metadata,
                fa_y_metadata,
            );

        let (usdt_reserve, btc_reserve) =
            router::get_reserves_size<USDT, BTC, Uncorrelated>(fa_y_metadata, fa_x_metadata);
        assert!(usdt_reserve == 10000, 0);
        assert!(btc_reserve == 100, 1);

        assert!(fungible_asset::amount(&coin_x) == x_out, 2);
        assert!(fungible_asset::amount(&coin_y) == y_out, 3);

        primary_fungible_store::deposit(signer::address_of(&lp_owner), coin_x);
        primary_fungible_store::deposit(signer::address_of(&lp_owner), coin_y);
    }

    // todo: recheck
    // TODO: test that one can't remove liquidity with reverse coin order
    //  (it's checked with generic params, so I don't know)

    #[test]
    #[expected_failure(abort_code = router::ERR_FA_OUT_NUM_LESS_THAN_EXPECTED_MINIMUM)]
    fun test_remove_liquidity_to_fail_if_less_than_minimum_x() {
        let (_, lp_owner) = register_pool_with_liquidity(101, 10100);

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"BTC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        let lp_metadata =
            liquidity_pool::get_pool_lp_metadata<BTC, USDT, Uncorrelated>(fa_x_metadata, fa_y_metadata);
        let lp_fa_val = 10;
        let lp_fa_to_burn =
            primary_fungible_store::withdraw(&lp_owner, lp_metadata, lp_fa_val);

        let (x_out, y_out) = router::get_reserves_for_lp_coins<BTC, USDT, Uncorrelated>(
            lp_fa_val,
            fa_x_metadata,
            fa_y_metadata,
        );
        let (coin_x, coin_y) =
            router::remove_liquidity<BTC, USDT, Uncorrelated>(
                lp_fa_to_burn,
                x_out * 2,
                y_out,
                fa_x_metadata,
                fa_y_metadata,
            );

        primary_fungible_store::deposit(signer::address_of(&lp_owner), coin_x);
        primary_fungible_store::deposit(signer::address_of(&lp_owner), coin_y);
    }

    #[test]
    #[expected_failure(abort_code = router::ERR_FA_OUT_NUM_LESS_THAN_EXPECTED_MINIMUM)]
    fun test_remove_liquidity_to_fail_if_less_than_minimum_y() {
        let (_, lp_owner) = register_pool_with_liquidity(101, 10100);

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"BTC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        let lp_metadata =
            liquidity_pool::get_pool_lp_metadata<BTC, USDT, Uncorrelated>(fa_x_metadata, fa_y_metadata);
        let lp_fa_val = 10;
        let lp_fa_to_burn =
            primary_fungible_store::withdraw(&lp_owner, lp_metadata, lp_fa_val);

        let (x_out, y_out) = router::get_reserves_for_lp_coins<BTC, USDT, Uncorrelated>(
            lp_fa_val,
            fa_x_metadata,
            fa_y_metadata,
        );
        let (coin_x, coin_y) =
            router::remove_liquidity<BTC, USDT, Uncorrelated>(
                lp_fa_to_burn,
                x_out,
                y_out * 2,
                fa_x_metadata,
                fa_y_metadata,
            );

        primary_fungible_store::deposit(signer::address_of(&lp_owner), coin_x);
        primary_fungible_store::deposit(signer::address_of(&lp_owner), coin_y);
    }

    #[test]
    #[expected_failure(abort_code = router::ERR_WRONG_FA_ORDER)]
    fun test_remove_liq_with_wrong_metadata_ordering_should_fail() {
        let (_, lp_owner) = register_pool_with_liquidity(101, 10100);

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"BTC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        let lp_metadata =
            liquidity_pool::get_pool_lp_metadata<BTC, USDT, Uncorrelated>(fa_x_metadata, fa_y_metadata);
        let lp_fa_val = 10;
        let lp_fa_to_burn =
            primary_fungible_store::withdraw(&lp_owner, lp_metadata, lp_fa_val);

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"BTC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        let (x_out, y_out) = router::get_reserves_for_lp_coins<BTC, USDT, Uncorrelated>(
            lp_fa_val,
            fa_x_metadata,
            fa_y_metadata,
        );
        let (coin_x, coin_y) =
            router::remove_liquidity<BTC, USDT, Uncorrelated>(
                lp_fa_to_burn,
                x_out,
                y_out,
                fa_y_metadata,
                fa_x_metadata,
            );

        assert!(fungible_asset::amount(&coin_x) == x_out, 2);
        assert!(fungible_asset::amount(&coin_y) == y_out, 3);
        primary_fungible_store::deposit(signer::address_of(&lp_owner), coin_x);
        primary_fungible_store::deposit(signer::address_of(&lp_owner), coin_y);
    }

    #[test]
    fun test_swap_exact_coin_for_coin() {
        let (fa_admin, _) = register_pool_with_liquidity(101, 10100);

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"BTC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        let btc_fa_swap_val = 1;
        let btc_fa_to_swap = test_coins::mint_fa(&fa_admin,  b"BTC", btc_fa_swap_val);
        let usdt_amount_out =
            router::get_amount_out<BTC, USDT, Uncorrelated>(btc_fa_swap_val, fa_x_metadata, fa_y_metadata);

        let usdt_fa = router::swap_exact_coin_for_coin<BTC, USDT, Uncorrelated>(
            btc_fa_to_swap,
            usdt_amount_out,
            fa_y_metadata,
        );
        assert!(fungible_asset::amount(&usdt_fa) == usdt_amount_out, 0);

        test_coins::burn_fa(&fa_admin, b"USDT", usdt_fa);
    }

    #[test]
    fun test_swap_exact_coin_for_coin_1() {
        let (fa_admin, _) = register_pool_with_liquidity(1230000000, 147600000000);

        let btc_to_swap_val = 572123800;
        let btc_fa_to_swap = test_coins::mint_fa(&fa_admin,  b"BTC", btc_to_swap_val);

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"BTC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        let usdt_to_get_val =
            router::get_amount_out<BTC, USDT, Uncorrelated>(btc_to_swap_val, fa_x_metadata, fa_y_metadata);

        let usdt_fa = router::swap_exact_coin_for_coin<BTC, USDT, Uncorrelated>(
            btc_fa_to_swap,
            usdt_to_get_val,
            fa_y_metadata,
        );
        assert!(fungible_asset::amount(&usdt_fa) == usdt_to_get_val, 0);

        test_coins::burn_fa(&fa_admin, b"USDT", usdt_fa);
    }

    #[test]
    fun test_swap_exact_coin_for_coin_2() {
        let (fa_admin, _) = register_pool_with_liquidity(10000000000, 2800000000000);

        let usdt_to_swap_val = 257817560;
        let usdt_to_swap = test_coins::mint_fa(&fa_admin,  b"USDT", usdt_to_swap_val);

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"BTC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        let btc_to_get_val =
            router::get_amount_out<USDT, BTC, Uncorrelated>(usdt_to_swap_val, fa_y_metadata, fa_x_metadata);

        let btc_fa = router::swap_exact_coin_for_coin<USDT, BTC, Uncorrelated>(
            usdt_to_swap,
            btc_to_get_val,
            fa_x_metadata
        );
        assert!(fungible_asset::amount(&btc_fa) == btc_to_get_val, 0);

        test_coins::burn_fa(&fa_admin, b"BTC", btc_fa);
    }

    #[test]
    fun test_swap_exact_coin_for_coin_reverse() {
        let (fa_admin, _) = register_pool_with_liquidity(101, 10100);

        let usdt_fa_to_swap = test_coins::mint_fa(&fa_admin,  b"USDT", 110);

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"BTC");

        let btc_fa = router::swap_exact_coin_for_coin<USDT, BTC, Uncorrelated>(
            usdt_fa_to_swap,
            1,
            fa_x_metadata,
        );
        assert!(fungible_asset::amount(&btc_fa) == 1, 0);

        test_coins::burn_fa(&fa_admin, b"BTC", btc_fa);
    }

    #[test]
    #[expected_failure(abort_code = router::ERR_FA_OUT_NUM_LESS_THAN_EXPECTED_MINIMUM)]
    fun test_swap_exact_coin_for_coin_to_fail_if_less_than_minimum_out() {
        let (fa_admin, _) = register_pool_with_liquidity(101, 10100);

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"BTC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        let btc_fa_swap_val = 1;
        let btc_fa_to_swap = test_coins::mint_fa(&fa_admin,  b"BTC", btc_fa_swap_val);
        let usdt_amount_out =
            router::get_amount_out<BTC, USDT, Uncorrelated>(btc_fa_swap_val, fa_x_metadata, fa_y_metadata);

        let usdt_fa = router::swap_exact_coin_for_coin<BTC, USDT, Uncorrelated>(
            btc_fa_to_swap,
            usdt_amount_out * 2,
            fa_y_metadata,
        );

        test_coins::burn_fa(&fa_admin, b"USDT", usdt_fa);
    }

    #[test]
    fun test_swap_coin_for_exact_coin() {
        let (fa_admin, _) = register_pool_with_liquidity(101, 10100);

        let btc_fa_to_swap = test_coins::mint_fa(&fa_admin,  b"BTC", 1);

        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        let (remainder, usdt_fa) =
            router::swap_coin_for_exact_coin<BTC, USDT, Uncorrelated>(
                btc_fa_to_swap,
                98,
                fa_y_metadata,
            );

        assert!(fungible_asset::amount(&usdt_fa) == 98, 0);
        assert!(fungible_asset::amount(&remainder) == 0, 1);

        test_coins::burn_fa(&fa_admin, b"USDT", usdt_fa);
        fungible_asset::destroy_zero(remainder);
    }

    #[test]
    fun test_swap_coin_for_exact_coin_1() {
        let (fa_admin, _) = register_pool_with_liquidity(50000000000, 13500000000000);

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"BTC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        let usdt_fa_to_get = 5292719411;
        let btc_fa_to_swap_val =
            router::get_amount_in<BTC, USDT, Uncorrelated>(usdt_fa_to_get, fa_x_metadata, fa_y_metadata);
        let btc_fa_to_swap = test_coins::mint_fa(&fa_admin,  b"BTC", btc_fa_to_swap_val);

        let (remainder, usdt_fa) =
            router::swap_coin_for_exact_coin<BTC, USDT, Uncorrelated>(
                btc_fa_to_swap,
                usdt_fa_to_get,
                fa_y_metadata,
            );

        assert!(fungible_asset::amount(&usdt_fa) == usdt_fa_to_get, 0);
        assert!(fungible_asset::amount(&remainder) == 0, 1);

        test_coins::burn_fa(&fa_admin, b"USDT", usdt_fa);
        fungible_asset::destroy_zero(remainder);
    }

    #[test]
    fun test_swap_coin_for_exact_coin_2() {
        let (fa_admin, _) = register_pool_with_liquidity(10000000000, 2800000000000);

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"BTC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        let btc_fa_to_get = 185200481;
        let usdt_fa_to_swap_val =
            router::get_amount_in<USDT, BTC, Uncorrelated>(btc_fa_to_get, fa_y_metadata, fa_x_metadata);

        let usdc_fa_to_swap = test_coins::mint_fa(&fa_admin,  b"USDT", usdt_fa_to_swap_val);

        let (remainder, btc_fa) =
            router::swap_coin_for_exact_coin<USDT, BTC, Uncorrelated>(
                usdc_fa_to_swap,
                btc_fa_to_get,
                fa_x_metadata,
            );

        assert!(fungible_asset::amount(&btc_fa) == btc_fa_to_get, 1);
        assert!(fungible_asset::amount(&remainder) == 0, 2);

        test_coins::burn_fa(&fa_admin, b"BTC", btc_fa);
        fungible_asset::destroy_zero(remainder);
    }

    #[test]
    #[expected_failure(abort_code = router::ERR_FA_VAL_MAX_LESS_THAN_NEEDED)]
    fun test_swap_coin_for_exact_coin_router_check_fails() {
        let (fa_admin, _) = register_pool_with_liquidity(10000000000, 2800000000000);

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"BTC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        let btc_fa_to_swap_val = 100000000;
        let btc_fa_to_swap = test_coins::mint_fa(&fa_admin,  b"BTC", btc_fa_to_swap_val);
        let usdt_to_get =
            router::get_amount_out<BTC, USDT, Uncorrelated>(btc_fa_to_swap_val, fa_x_metadata, fa_y_metadata) + 1;

        let (remainder, usdt_fa) =
            router::swap_coin_for_exact_coin<BTC, USDT, Uncorrelated>(
                btc_fa_to_swap,
                usdt_to_get,
                fa_y_metadata,
            );

        assert!(fungible_asset::amount(&usdt_fa) == usdt_to_get, 1);
        assert!(fungible_asset::amount(&remainder) == 0, 2);

        test_coins::burn_fa(&fa_admin, b"USDT", usdt_fa);
        fungible_asset::destroy_zero(remainder);
    }

    #[test]
    fun test_swap_coin_for_exact_coin_reverse() {
        let (fa_admin, _) = register_pool_with_liquidity(101, 10100);

        let usdt_fa_to_swap = test_coins::mint_fa(&fa_admin,  b"USDT", 1114);

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"BTC");

        let (remainder, btc_fa) =
            router::swap_coin_for_exact_coin<USDT, BTC, Uncorrelated>(
                usdt_fa_to_swap,
                10,
                fa_x_metadata,
            );

        assert!(fungible_asset::amount(&btc_fa) == 10, 0);
        assert!(fungible_asset::amount(&remainder) == 0, 1);

        test_coins::burn_fa(&fa_admin, b"BTC", btc_fa);
        fungible_asset::destroy_zero(remainder);
    }

    #[test]
    #[expected_failure(abort_code = router::ERR_FA_OUT_NUM_LESS_THAN_EXPECTED_MINIMUM)]
    fun test_fail_if_price_fell_behind_threshold() {
        let (fa_admin, lp_owner) = register_pool_with_liquidity(101, 10100);

        let btc_coin_to_swap = test_coins::mint_fa(&fa_admin,  b"BTC", 1);

        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        let usdt_fa =
            router::swap_exact_coin_for_coin<BTC, USDT, Uncorrelated>(
                btc_coin_to_swap,
                102,
                fa_y_metadata,
            );

        primary_fungible_store::deposit(signer::address_of(&lp_owner), usdt_fa);
    }

    #[test]
    #[expected_failure(abort_code = liquidity_pool::ERR_EMPTY_FA_IN)]
    fun test_fail_if_swap_zero_coin() {
        let (fa_admin, lp_owner) = register_pool_with_liquidity(101, 10100);

        let btc_fa_to_swap = test_coins::mint_fa(&fa_admin,  b"BTC", 0);

        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        let usdt_fa =
            router::swap_exact_coin_for_coin<BTC, USDT, Uncorrelated>(
                btc_fa_to_swap,
                0,
                fa_y_metadata,
            );

        primary_fungible_store::deposit(signer::address_of(&lp_owner), usdt_fa);
    }

    #[test]
    fun test_returned_usdt_proportially_decrease_for_big_swaps() {
        let (fa_admin, lp_owner) = register_pool_with_liquidity(101, 10100);

        let btc_fa_to_swap = test_coins::mint_fa(&fa_admin,  b"BTC", 200);

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"BTC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        let usdt_fa =
            router::swap_exact_coin_for_coin<BTC, USDT, Uncorrelated>(
                btc_fa_to_swap,
                1,
                fa_y_metadata
            );
        assert!(fungible_asset::amount(&usdt_fa) == 6704, 0);

        let (btc_reserve, usdt_reserve) =
            router::get_reserves_size<BTC, USDT, Uncorrelated>(fa_x_metadata, fa_y_metadata);
        assert!(btc_reserve == 301, 1);
        assert!(usdt_reserve == 3396, 2);
        assert!(router::current_price<USDT, BTC, Uncorrelated>(fa_y_metadata, fa_x_metadata) == 11, 3);

        primary_fungible_store::deposit(signer::address_of(&lp_owner), usdt_fa);
    }

    #[test]
    fun test_pool_exists() {
        let (_, lp_owner) = test_pool::setup_fa_and_lp_owner();

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"BTC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        router::register_pool<BTC, USDT, Uncorrelated>(&lp_owner, fa_x_metadata, fa_y_metadata);

        assert!(router::is_swap_exists<BTC, USDT, Uncorrelated>(fa_x_metadata, fa_y_metadata), 0);
        assert!(router::is_swap_exists<USDT, BTC, Uncorrelated>(fa_y_metadata, fa_x_metadata), 1);
    }

    #[test]
    fun test_cumulative_prices_after_swaps() {
        let (fa_admin, lp_owner) = register_pool_with_liquidity(101, 10100);

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"BTC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        let (btc_price, usdt_price, ts) =
            router::get_cumulative_prices<BTC, USDT, Uncorrelated>(fa_x_metadata, fa_y_metadata);
        assert!(btc_price == 0, 0);
        assert!(usdt_price == 0, 1);
        assert!(ts == 0, 2);

        // 2 seconds
        timestamp::update_global_time_for_test(2000000);

        let btc_to_swap = test_coins::mint_fa(&fa_admin,  b"BTC", 1);
        let usdts =
            router::swap_exact_coin_for_coin<BTC, USDT, Uncorrelated>(
                btc_to_swap,
                95,
                fa_y_metadata,
            );
        primary_fungible_store::deposit(signer::address_of(&lp_owner), usdts);

        let (btc_cum_price, usdt_cum_price, last_timestamp) =
            router::get_cumulative_prices<BTC, USDT, Uncorrelated>(fa_x_metadata, fa_y_metadata);
        assert!(btc_cum_price == 3689348814741910323000, 3);
        assert!(usdt_cum_price == 368934881474191032, 4);
        assert!(last_timestamp == 2, 5);

        // Small test for commulative prices reverse
        let (usdt_cum_price, btc_cum_price, last_timestamp) =
            router::get_cumulative_prices<USDT, BTC, Uncorrelated>(fa_y_metadata, fa_x_metadata);
        assert!(btc_cum_price == 3689348814741910323000, 3);
        assert!(usdt_cum_price == 368934881474191032, 4);
        assert!(last_timestamp == 2, 5);

        // 4 seconds
        timestamp::update_global_time_for_test(4000000);

        let btc_to_swap = test_coins::mint_fa(&fa_admin,  b"BTC", 2);
        let usdts =
            router::swap_exact_coin_for_coin<BTC, USDT, Uncorrelated>(
                btc_to_swap,
                190,
                fa_y_metadata,
            );
        primary_fungible_store::deposit(signer::address_of(&lp_owner), usdts);

        let (btc_cum_price, usdt_cum_price, last_timestamp) =
            router::get_cumulative_prices<BTC, USDT, Uncorrelated>(fa_x_metadata, fa_y_metadata);
        assert!(btc_cum_price == 7307080858374124739730, 6);
        assert!(usdt_cum_price == 745173212911578406, 7);
        assert!(last_timestamp == 4, 8);
    }

    #[test]
    fun test_stable_curve_swap_exact() {
        let (fa_admin, lp_owner) =
            register_stable_pool_with_liquidity(15000000000, 1500000000000);

        let usdc_to_swap_val = 1258044;

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"USDC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        let usdc_to_swap = test_coins::mint_fa(&fa_admin,  b"USDC", usdc_to_swap_val);
        let usdt_to_get =
            router::get_amount_out<USDC, USDT, Stable>(usdc_to_swap_val, fa_x_metadata, fa_y_metadata);

        let usdt_swapped = router::swap_exact_coin_for_coin<USDC, USDT, Stable>(
            usdc_to_swap,
            usdt_to_get,
            fa_y_metadata,
        );

        assert!(fungible_asset::amount(&usdt_swapped) == usdt_to_get, 1);

        primary_fungible_store::deposit(signer::address_of(&lp_owner), usdt_swapped);
    }

    #[test]
    fun test_stable_curve_swap_exact_1() {
        let (fa_admin, lp_owner) =
            register_stable_pool_with_liquidity(15000000000, 1500000000000);

        let usdc_to_swap_val = 67482132;

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"USDC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        let usdc_to_swap = test_coins::mint_fa(&fa_admin,  b"USDC", usdc_to_swap_val);
        let usdt_to_get =
            router::get_amount_out<USDC, USDT, Stable>(usdc_to_swap_val, fa_x_metadata, fa_y_metadata);

        let usdt_swapped = router::swap_exact_coin_for_coin<USDC, USDT, Stable>(
            usdc_to_swap,
            usdt_to_get,
            fa_y_metadata,
        );
        assert!(fungible_asset::amount(&usdt_swapped) == usdt_to_get, 1);

        primary_fungible_store::deposit(signer::address_of(&lp_owner), usdt_swapped);
    }

    #[test]
    fun test_stable_curve_swap_exact_2() {
        let (fa_admin, lp_owner) =
            register_stable_pool_with_liquidity(15000000000, 1500000000000);

        let usdc_to_swap_val = 1207482132;

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"USDC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        let usdc_to_swap = test_coins::mint_fa(&fa_admin,  b"USDC", usdc_to_swap_val);
        let usdt_to_get =
            router::get_amount_out<USDC, USDT, Stable>(usdc_to_swap_val, fa_x_metadata, fa_y_metadata);

        let usdt_swapped = router::swap_exact_coin_for_coin<USDC, USDT, Stable>(
            usdc_to_swap,
            usdt_to_get,
            fa_y_metadata,
        );
        assert!(fungible_asset::amount(&usdt_swapped) == usdt_to_get, 1);

        primary_fungible_store::deposit(signer::address_of(&lp_owner), usdt_swapped);
    }

    #[test]
    fun test_stable_curve_swap_exact_3() {
        let (fa_admin, lp_owner) =
            register_stable_pool_with_liquidity(2930000000000, 293000000000000);

        let usdc_to_swap_val = 32207482132;

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"USDC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        let usdc_to_swap = test_coins::mint_fa(&fa_admin,  b"USDC", usdc_to_swap_val);
        let usdt_to_get =
            router::get_amount_out<USDC, USDT, Stable>(usdc_to_swap_val, fa_x_metadata, fa_y_metadata);

        let usdt_swapped = router::swap_exact_coin_for_coin<USDC, USDT, Stable>(
            usdc_to_swap,
            usdt_to_get,
            fa_y_metadata,
        );
        assert!(fungible_asset::amount(&usdt_swapped) == usdt_to_get, 1);

        primary_fungible_store::deposit(signer::address_of(&lp_owner), usdt_swapped);
    }

    #[test]
    fun test_stable_curve_swap_exact_vice_versa() {
        let (fa_admin, lp_owner) =
            register_stable_pool_with_liquidity(15000000000, 1500000000000);

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"USDC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        // Let's swap USDT -> USDC.
        let usdt_to_swap_val = 1254269;
        let usdc_to_get_val =
            router::get_amount_out<USDT, USDC, Stable>(usdt_to_swap_val, fa_y_metadata, fa_x_metadata);
        let usdt_to_swap = test_coins::mint_fa(&fa_admin,  b"USDT", usdt_to_swap_val);

        let usdc_swapped = router::swap_exact_coin_for_coin<USDT, USDC, Stable>(
            usdt_to_swap,
            usdc_to_get_val,
            fa_x_metadata,
        );

        assert!(fungible_asset::amount(&usdc_swapped) == usdc_to_get_val, 1);

        primary_fungible_store::deposit(signer::address_of(&lp_owner), usdc_swapped);
    }

    #[test]
    fun test_stable_curve_swap_exact_vice_versa_1() {
        let (fa_admin, lp_owner) =
            register_stable_pool_with_liquidity(15000000000, 1500000000000);

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"USDC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        let usdt_to_swap_val = 125426939;
        let usdc_to_get_val =
            router::get_amount_out<USDT, USDC, Stable>(usdt_to_swap_val, fa_y_metadata, fa_x_metadata);
        let usdt_to_swap = test_coins::mint_fa(&fa_admin,  b"USDT", usdt_to_swap_val);

        let usdc_swapped = router::swap_exact_coin_for_coin<USDT, USDC, Stable>(
            usdt_to_swap,
            usdc_to_get_val,
            fa_x_metadata,
        );

        assert!(fungible_asset::amount(&usdc_swapped) == usdc_to_get_val, 1);

        primary_fungible_store::deposit(signer::address_of(&lp_owner), usdc_swapped);
    }

    #[test]
    fun test_stable_curve_exact_swap() {
        let (fa_admin, lp_owner) =
            register_stable_pool_with_liquidity(15000000000, 1500000000000);

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"USDC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        let usdc_to_get_val = 1254269;
        let usdt_to_swap_val =
            router::get_amount_in<USDT, USDC, Stable>(usdc_to_get_val, fa_y_metadata, fa_x_metadata);

        let usdt_to_swap = test_coins::mint_fa(&fa_admin,  b"USDT", usdt_to_swap_val);
        let (usdt_reminder, usdc_swapped) =
            router::swap_coin_for_exact_coin<USDT, USDC, Stable>(
                usdt_to_swap,
                usdc_to_get_val,
                fa_x_metadata,
            );

        assert!(fungible_asset::amount(&usdt_reminder) == 0, 1);
        assert!(fungible_asset::amount(&usdc_swapped) == usdc_to_get_val, 2);

        primary_fungible_store::deposit(signer::address_of(&lp_owner), usdt_reminder);
        primary_fungible_store::deposit(signer::address_of(&lp_owner), usdc_swapped);
    }

    #[test]
    fun test_stable_curve_exact_swap_vice_versa() {
        let (fa_admin, lp_owner) =
            register_stable_pool_with_liquidity(15000000000, 1500000000000);

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"USDC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        let usdt_to_get_val = 125804401;
        let usdc_to_swap_val =
            router::get_amount_in<USDC, USDT, Stable>(usdt_to_get_val, fa_x_metadata, fa_y_metadata);

        let usdc_to_swap = test_coins::mint_fa(&fa_admin,  b"USDC", usdc_to_swap_val);
        let (usdc_reminder, usdt_swapped) =
            router::swap_coin_for_exact_coin<USDC, USDT, Stable>(
                usdc_to_swap,
                usdt_to_get_val,
                fa_y_metadata,
            );

        assert!(fungible_asset::amount(&usdc_reminder) == 0, 1);
        assert!(fungible_asset::amount(&usdt_swapped) == usdt_to_get_val, 2);

        primary_fungible_store::deposit(signer::address_of(&lp_owner), usdc_reminder);
        primary_fungible_store::deposit(signer::address_of(&lp_owner), usdt_swapped);
    }

    #[test]
    fun test_stable_get_amount_in() {
        let (_, _) = register_stable_pool_with_liquidity(15000000000, 1500000000000);

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"USDC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        let amount_in =
            router::get_amount_in<USDC, USDT, Stable>(67279092, fa_x_metadata, fa_y_metadata);
        assert!(amount_in == 673061, 0);
    }

    #[test]
    fun test_get_amount_in() {
        let (_, _) = register_pool_with_liquidity(10828583259, 2764800200409);

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"BTC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        let amount_in =
            router::get_amount_in<USDT, BTC, Uncorrelated>(158202011, fa_y_metadata, fa_x_metadata);
        assert!(amount_in == 41115034299, 0);
    }

    #[test]
    fun test_get_amount_in_1() {
        let (_, _) = register_pool_with_liquidity(10828583259, 2764800200409);

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"BTC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        let amount_in =
            router::get_amount_in<BTC, USDT, Uncorrelated>(28253021000, fa_x_metadata, fa_y_metadata);
        assert!(amount_in == 112134290, 0);
    }

    #[test]
    fun test_get_amount_in_2() {
        let (_, _) = register_pool_with_liquidity(10828583259, 2764800200409);

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"BTC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        let amount_in =
            router::get_amount_in<USDT, BTC, Uncorrelated>(1, fa_y_metadata, fa_x_metadata);
        assert!(amount_in == 257, 0);
    }

    #[test]
    fun test_stable_get_amount_in_1() {
        let (_, _) = register_stable_pool_with_liquidity(20000000000, 1000000000000);

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"USDC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        let amount_in =
            router::get_amount_in<USDC, USDT, Stable>(67279092, fa_x_metadata, fa_y_metadata);
        assert!(amount_in == 724846, 0);
    }

    #[test]
    fun test_stable_get_amount_in_2() {
        let (_, _) = register_stable_pool_with_liquidity(15000000000, 1500000000000);

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"USDC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        let amount_in =
            router::get_amount_in<USDC, USDT, Stable>(15000, fa_x_metadata, fa_y_metadata);
        assert!(amount_in == 152, 0);
    }

    #[test]
    fun test_stable_get_amount_in_3() {
        let (_, _) = register_stable_pool_with_liquidity(15000000000, 1500000000000);

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"USDC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        let amount_in =
            router::get_amount_in<USDT, USDC, Stable>(158282982, fa_y_metadata, fa_x_metadata);
        assert!(amount_in == 15834641356, 0);
    }

    #[test]
    fun test_stable_get_amount_in_4() {
        let (_, _) = register_stable_pool_with_liquidity(15000000000, 1500000000000);

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"USDC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        let amount_in =
            router::get_amount_in<USDT, USDC, Stable>(1, fa_y_metadata, fa_x_metadata);
        assert!(amount_in == 102, 0);
    }

    #[test]
    fun test_stable_get_amount_in_5() {
        let (_, _) = register_stable_pool_with_liquidity(2930000000000, 293000000000000);

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"USDC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        let amount_in =
            router::get_amount_in<USDT, USDC, Stable>(57212828231, fa_y_metadata, fa_x_metadata);
        assert!(amount_in == 5723593558779, 0);
    }

    #[test]
    fun test_stable_get_amount_out() {
        let (_, _) = register_stable_pool_with_liquidity(15000000000, 1500000000000);

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"USDC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        let amount_out =
            router::get_amount_out<USDC, USDT, Stable>(674816, fa_x_metadata, fa_y_metadata);
        assert!(amount_out == 67454699, 0);
    }

    #[test]
    fun test_get_amount_out() {
        let (_, _) = register_pool_with_liquidity(18000000000, 4680000000000);

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"BTC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        let amount_out =
            router::get_amount_out<USDT, BTC, Uncorrelated>(1500000000, fa_y_metadata, fa_x_metadata);
        assert!(amount_out == 5750085, 0);
    }

    #[test]
    fun test_get_amount_out_1() {
        let (_, _) = register_pool_with_liquidity(18000000000, 4680000000000);

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"BTC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        let amount_out =
            router::get_amount_out<BTC, USDT, Uncorrelated>(100000000, fa_x_metadata, fa_y_metadata);
        assert!(amount_out == 25779211810, 0);
    }

    #[test]
    fun test_get_amount_out_2() {
        let (_, _) = register_pool_with_liquidity(18000000000, 4680000000000);

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"BTC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        let amount_out =
            router::get_amount_out<BTC, USDT, Uncorrelated>(1, fa_x_metadata, fa_y_metadata);
        assert!(amount_out == 259, 0);
    }

    #[test]
    fun test_stable_get_amount_out_1() {
        let (_, _) = register_stable_pool_with_liquidity(25000000000, 1500000000000);

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"USDC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        let amount_out =
            router::get_amount_out<USDC, USDT, Stable>(323859, fa_x_metadata, fa_y_metadata);
        assert!(amount_out == 31376814, 0);
    }

    #[test]
    fun test_stable_get_amount_out_2() {
        let (_, _) = register_stable_pool_with_liquidity(15000000000, 1500000000000);

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"USDC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        let amount_out =
            router::get_amount_out<USDC, USDT, Stable>(58201, fa_x_metadata, fa_y_metadata);
        assert!(amount_out == 5817799, 0);
    }

    #[test]
    fun test_stable_get_amount_out_3() {
        let (_, _) = register_stable_pool_with_liquidity(15000000000, 1500000000000);

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"USDC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        let amount_out =
            router::get_amount_out<USDT, USDC, Stable>(15000, fa_y_metadata, fa_x_metadata);
        assert!(amount_out == 149, 0);
    }

    #[test]
    fun test_stable_get_amount_out_4() {
        let (_, _) = register_stable_pool_with_liquidity(15000000000, 1500000000000);

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"USDC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        let amount_out =
            router::get_amount_out<USDT, USDC, Stable>(1, fa_y_metadata, fa_x_metadata);
        assert!(amount_out == 0, 0);
    }

    #[test]
    fun test_stable_get_amount_out_5() {
        let (_, _) = register_stable_pool_with_liquidity(2930000000000, 293000000000000);

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"USDC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        let amount_out =
            router::get_amount_out<USDT, USDC, Stable>(572123482812, fa_y_metadata, fa_x_metadata);
        assert!(amount_out == 5718946312, 0);
    }

    #[test]
    fun test_stable_curve_exact_swap_vice_vera_1() {
        let (fa_admin, lp_owner) = register_stable_pool_with_liquidity(15000000000, 1500000000000);

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"USDC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        let usdt_to_get_val = 672790928312;
        let usdc_to_swap_val =
            router::get_amount_in<USDC, USDT, Stable>(usdt_to_get_val, fa_x_metadata, fa_y_metadata);

        let usdc_to_swap = test_coins::mint_fa(&fa_admin,  b"USDC", usdc_to_swap_val);
        let (usdc_reminder, usdt_swapped) =
            router::swap_coin_for_exact_coin<USDC, USDT, Stable>(
                usdc_to_swap,
                usdt_to_get_val,
                fa_y_metadata,
            );

        assert!(fungible_asset::amount(&usdc_reminder) == 0, 1);
        assert!(fungible_asset::amount(&usdt_swapped) == usdt_to_get_val, 2);

        primary_fungible_store::deposit(signer::address_of(&lp_owner), usdc_reminder);
        primary_fungible_store::deposit(signer::address_of(&lp_owner), usdt_swapped);
    }

    #[test]
    fun test_stable_curve_exact_swap_vice_versa_2() {
        let (fa_admin, lp_owner) = register_stable_pool_with_liquidity(15000000000, 1500000000000);

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"USDC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        let usdt_to_get_val = 672790928;
        let usdc_to_swap_val =
            router::get_amount_in<USDC, USDT, Stable>(usdt_to_get_val, fa_x_metadata, fa_y_metadata);

        let usdc_to_swap = test_coins::mint_fa(&fa_admin,  b"USDC", usdc_to_swap_val);
        let (usdc_reminder, usdt_swapped) = router::swap_coin_for_exact_coin<USDC, USDT, Stable>(
            usdc_to_swap,
            usdt_to_get_val,
            fa_y_metadata,
        );

        assert!(fungible_asset::amount(&usdc_reminder) == 0, 1);
        assert!(fungible_asset::amount(&usdt_swapped) == usdt_to_get_val, 2);

        primary_fungible_store::deposit(signer::address_of(&lp_owner), usdc_reminder);
        primary_fungible_store::deposit(signer::address_of(&lp_owner), usdt_swapped);
    }

    #[test]
    fun test_convert_with_current_price() {
        let a = router::convert_with_current_price(MAX_U64, MAX_U64, MAX_U64);
        assert!(a == MAX_U64, 0);

        a = router::convert_with_current_price(100, 100, 20);
        assert!(a == 20, 1);

        a = router::convert_with_current_price(256, 8, 2);
        assert!(a == 64, 1);
    }

    #[test]
    #[expected_failure(abort_code = router::ERR_WRONG_AMOUNT)]
    fun test_fail_convert_with_current_price_coin_in_val() {
        router::convert_with_current_price(0, 1, 1);
    }

    #[test]
    #[expected_failure(abort_code = router::ERR_WRONG_RESERVE)]
    fun test_fail_convert_with_current_price_reserve_in_size() {
        router::convert_with_current_price(1, 0, 1);
    }

    #[test]
    #[expected_failure(abort_code = router::ERR_WRONG_RESERVE)]
    fun test_fail_convert_with_current_price_reserve_out_size() {
        router::convert_with_current_price(1, 1, 0);
    }

    #[test]
    fun test_get_decimals_scales_stables() {
        let (_, _) = register_stable_pool_with_liquidity(15000000000, 1500000000000);

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"USDC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        let (x, y) = router::get_decimals_scales<USDC, USDT, Stable>(fa_x_metadata, fa_y_metadata);

        // USDC 4 decimals
        assert!(x == 10000, 0);
        // USDT 6 decimals
        assert!(y == 1000000, 1);
    }

    #[test]
    fun test_get_decimals_scales_stables_reverse() {
        let (_, _) = register_stable_pool_with_liquidity(15000000000, 1500000000000);

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"USDC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        let (x, y) = router::get_decimals_scales<USDT, USDC, Stable>(fa_y_metadata, fa_x_metadata);


        // USDT 6 decimals
        assert!(x == 1000000, 1);
        // USDC 4 decimals
        assert!(y == 10000, 0);
    }

    #[test]
    fun test_get_decimals_scales_uncorrelated() {
        let (_, _) = register_pool_with_liquidity(101, 10100);

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"BTC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        let (x, y) = router::get_decimals_scales<BTC, USDT, Uncorrelated>(fa_x_metadata, fa_y_metadata);

        assert!(x == 0, 0);
        assert!(y == 0, 1);
    }


    #[test]
    fun test_swap_coin_for_coin_unchecked() {
        let (fa_admin, _) = register_pool_with_liquidity(101, 10100);

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"BTC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        let btc_fa_swap_val = 1;
        let btc_fa_to_swap = test_coins::mint_fa(&fa_admin,  b"BTC", btc_fa_swap_val);
        let usdt_amount_out =
            router::get_amount_out<BTC, USDT, Uncorrelated>(btc_fa_swap_val, fa_x_metadata, fa_y_metadata);

        let usdt_fa = router::swap_coin_for_coin_unchecked<BTC, USDT, Uncorrelated>(
            btc_fa_to_swap,
            usdt_amount_out,
            fa_y_metadata,
        );
        assert!(fungible_asset::amount(&usdt_fa) == usdt_amount_out, 0);

        test_coins::burn_fa(&fa_admin, b"USDT", usdt_fa);
    }

    #[test]
    fun test_stable_swap_coin_for_coin_unchecked() {
        let (fa_admin, _) = register_stable_pool_with_liquidity(150000000, 15000000000);

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"USDC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        let usdc_fa_swap_val = 100;
        let usdc_fa_to_swap = test_coins::mint_fa(&fa_admin,  b"USDC", usdc_fa_swap_val);
        let usdt_amount_out =
            router::get_amount_out<USDC, USDT, Stable>(usdc_fa_swap_val, fa_x_metadata, fa_y_metadata);

        let usdt_fa = router::swap_coin_for_coin_unchecked<USDC, USDT, Stable>(
            usdc_fa_to_swap,
            usdt_amount_out,
            fa_y_metadata,
        );
        assert!(fungible_asset::amount(&usdt_fa) == usdt_amount_out, 0);

        test_coins::burn_fa(&fa_admin, b"USDT", usdt_fa);
    }

    #[test]
    fun test_stable_swap_coin_for_coin_unchecked_reverse() {
        let (fa_admin, _) = register_stable_pool_with_liquidity(150000000, 15000000000);

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"USDC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        let usdt_to_swap_val = 10000;
        let usdt_to_swap = test_coins::mint_fa(&fa_admin,  b"USDT", usdt_to_swap_val);
        let usdt_amount_out =
            router::get_amount_out<USDT, USDC, Stable>(usdt_to_swap_val, fa_y_metadata, fa_x_metadata);

        let usdc_fa = router::swap_coin_for_coin_unchecked<USDT, USDC, Stable>(
            usdt_to_swap,
            usdt_amount_out,
            fa_x_metadata,
        );
        assert!(fungible_asset::amount(&usdc_fa) == usdt_amount_out, 0);

        test_coins::burn_fa(&fa_admin, b"USDC", usdc_fa);
    }

    #[test]
    fun test_swap_coin_for_coin_unchecked_reverse() {
        let (fa_admin, _) = register_pool_with_liquidity(101, 10100);

        let usdt_fa_to_swap = test_coins::mint_fa(&fa_admin,  b"USDT", 110);

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"BTC");

        let btc_fa = router::swap_coin_for_coin_unchecked<USDT, BTC, Uncorrelated>(
            usdt_fa_to_swap,
            1,
            fa_x_metadata,
        );
        assert!(fungible_asset::amount(&btc_fa) == 1, 0);

        test_coins::burn_fa(&fa_admin, b"BTC", btc_fa);
    }

    #[test]
    fun test_swap_coin_for_coin_unchecked_1() {
        let (fa_admin, _) = register_pool_with_liquidity(1230000000, 147600000000);

        let btc_to_swap_val = 572123800;
        let btc_fa_to_swap = test_coins::mint_fa(&fa_admin,  b"BTC", btc_to_swap_val);

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"BTC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        let usdt_to_get_val =
            router::get_amount_out<BTC, USDT, Uncorrelated>(btc_to_swap_val, fa_x_metadata, fa_y_metadata) - 1;

        let usdt_fa = router::swap_coin_for_coin_unchecked<BTC, USDT, Uncorrelated>(
            btc_fa_to_swap,
            usdt_to_get_val,
            fa_y_metadata,
        );
        assert!(fungible_asset::amount(&usdt_fa) == usdt_to_get_val, 0);

        test_coins::burn_fa(&fa_admin, b"USDT", usdt_fa);
    }

    #[test]
    fun test_swap_coin_for_coin_unchecked_2() {
        let (fa_admin, _) = register_pool_with_liquidity(10000000000, 2800000000000);

        let usdt_to_swap_val = 257817560;
        let usdt_to_swap = test_coins::mint_fa(&fa_admin,  b"USDT", usdt_to_swap_val);

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"BTC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        let btc_to_get_val =
            router::get_amount_out<USDT, BTC, Uncorrelated>(usdt_to_swap_val, fa_y_metadata, fa_x_metadata) - 134567;

        let btc_fa = router::swap_coin_for_coin_unchecked<USDT, BTC, Uncorrelated>(
            usdt_to_swap,
            btc_to_get_val,
            fa_x_metadata,
        );
        assert!(fungible_asset::amount(&btc_fa) == btc_to_get_val, 0);

        test_coins::burn_fa(&fa_admin, b"BTC", btc_fa);
    }

    #[test]
    #[expected_failure(abort_code = liquidity_pool::ERR_INCORRECT_SWAP)]
    fun test_fail_if_price_fell_behind_threshold_unchecked() {
        let (fa_admin, lp_owner) = register_pool_with_liquidity(101, 10100);

        let btc_coin_to_swap = test_coins::mint_fa(&fa_admin,  b"BTC", 1);

        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        let usdt_fa =
            router::swap_coin_for_coin_unchecked<BTC, USDT, Uncorrelated>(
                btc_coin_to_swap,
                102,
                fa_y_metadata,
            );

        primary_fungible_store::deposit(signer::address_of(&lp_owner), usdt_fa);
    }

    #[test]
    #[expected_failure(abort_code = liquidity_pool::ERR_INCORRECT_SWAP)]
    fun test_stable_fail_if_price_fell_behind_threshold_unchecked() {
        let (fa_admin, _) = register_stable_pool_with_liquidity(150000000, 15000000000);

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"USDC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        let usdc_fa_swap_val = 100;
        let usdc_fa_to_swap = test_coins::mint_fa(&fa_admin,  b"USDC", usdc_fa_swap_val);
        let usdt_amount_out =
            router::get_amount_out<USDC, USDT, Stable>(usdc_fa_swap_val, fa_x_metadata, fa_y_metadata);

        let usdt_fa = router::swap_coin_for_coin_unchecked<USDC, USDT, Stable>(
            usdc_fa_to_swap,
            usdt_amount_out + 1,
            fa_y_metadata,
        );
        assert!(fungible_asset::amount(&usdt_fa) == usdt_amount_out, 0);

        test_coins::burn_fa(&fa_admin, b"USDT", usdt_fa);
    }

    #[test]
    #[expected_failure(abort_code = liquidity_pool::ERR_INCORRECT_SWAP)]
    fun test_stable_fail_if_price_fell_behind_threshold_unchecked_1() {
        let (fa_admin, _) = register_stable_pool_with_liquidity(150000000, 15000000000);

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"USDC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        let usdc_fa_swap_val = 999999;
        let usdc_fa_to_swap = test_coins::mint_fa(&fa_admin,  b"USDC", usdc_fa_swap_val);
        let usdt_amount_out =
            router::get_amount_out<USDC, USDT, Stable>(usdc_fa_swap_val, fa_x_metadata, fa_y_metadata);

        let usdt_fa = router::swap_coin_for_coin_unchecked<USDC, USDT, Stable>(
            usdc_fa_to_swap,
            usdt_amount_out + 1,
            fa_y_metadata,
        );
        assert!(fungible_asset::amount(&usdt_fa) == usdt_amount_out, 0);

        test_coins::burn_fa(&fa_admin, b"USDT", usdt_fa);
    }

    #[test]
    #[expected_failure(abort_code = liquidity_pool::ERR_INCORRECT_SWAP)]
    fun test_stable_fail_if_price_fell_behind_threshold_unchecked_2() {
        let (fa_admin, _) = register_stable_pool_with_liquidity(150000000, 15000000000);

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"USDC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        let usdc_fa_to_get = 999999;
        let usdt_fa_to_swap_val =
            router::get_amount_in<USDT, USDC, Stable>(usdc_fa_to_get, fa_y_metadata, fa_x_metadata);
        let usdt_fa_to_swap = test_coins::mint_fa(&fa_admin,  b"USDT", usdt_fa_to_swap_val);

        let btc_fa = router::swap_coin_for_coin_unchecked<USDT, USDC, Stable>(
            usdt_fa_to_swap,
            usdc_fa_to_get + 1,
            fa_x_metadata,
        );
        assert!(fungible_asset::amount(&btc_fa) == usdc_fa_to_get, 0);

        test_coins::burn_fa(&fa_admin, b"BTC", btc_fa);
    }

    #[test]
    #[expected_failure(abort_code = liquidity_pool::ERR_INCORRECT_SWAP)]
    fun test_swap_coin_for_coin_unchecked_fails() {
        let (fa_admin, _) = register_pool_with_liquidity(10000000000, 2800000000000);

        let usdt_to_swap_val = 257817560;
        let usdt_to_swap = test_coins::mint_fa(&fa_admin,  b"USDT", usdt_to_swap_val);

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"BTC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        let btc_to_get_val =
            router::get_amount_out<USDT, BTC, Uncorrelated>(usdt_to_swap_val, fa_y_metadata, fa_x_metadata) + 1;

        let btc_fa = router::swap_coin_for_coin_unchecked<USDT, BTC, Uncorrelated>(
            usdt_to_swap,
            btc_to_get_val,
            fa_x_metadata,
        );
        assert!(fungible_asset::amount(&btc_fa) == btc_to_get_val, 0);

        test_coins::burn_fa(&fa_admin, b"BTC", btc_fa);
    }

    #[test]
    fun test_calc_optimal_coin_values() {
        // 100 BTC, 2,800,000 USDT
        let (_, _) = register_pool_with_liquidity(10000000000, 2800000000000);

        // 1 BTC, 10000 USDT
        let x_desired = 100000000;
        let y_desired = 10000000000;

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"BTC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        let (x_value, y_value) =
            router::calc_optimal_coin_values<BTC, USDT, Uncorrelated>(
                x_desired, y_desired,
                0,
                0,
                fa_x_metadata,
                fa_y_metadata,
            );

        // 1e8 x 1e10 / (2.8 x 1e11) = 35714285
        assert!(x_value == 35714285, 0);
        assert!(y_value == y_desired, 1);
    }

    #[test]
    #[expected_failure(abort_code = router::ERR_INSUFFICIENT_X_AMOUNT)]
    fun test_calc_optimal_coin_values_1() {
        // 100 BTC, 2,800,000 USDT
        let (_, _) = register_pool_with_liquidity(10000000000, 2800000000000);

        // 1 BTC, 10000 USDT
        let x_desired = 100000000;
        let y_desired = 10000000000;

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"BTC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        let (_, _) =
            router::calc_optimal_coin_values<BTC, USDT, Uncorrelated>(
                x_desired,
                y_desired,
                5000000000,
                0,
                fa_x_metadata,
                fa_y_metadata,
            );
    }

    #[test]
    fun test_calc_optimal_coin_values_3() {
        // 100 BTC, 28000 USDT
        let (_, _) = register_pool_with_liquidity(10000000000, 28000000000);

        // 1 BTC, 10000 USDT
        let x_desired = 100000000;
        let y_desired = 10000000000;

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"BTC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        let (x_res, y_res) =
            router::get_reserves_size<BTC, USDT, Uncorrelated>(fa_x_metadata, fa_y_metadata);

        let (x_value, y_value) =
            router::calc_optimal_coin_values<BTC, USDT, Uncorrelated>(
                x_desired,
                y_desired,
                0,
                0,
                fa_x_metadata,
                fa_y_metadata,
            );

        assert!(x_value == x_desired, 0);
        assert!(y_value == x_desired * y_res / x_res, 1);
    }

    #[test]
    #[expected_failure(abort_code = router::ERR_INSUFFICIENT_Y_AMOUNT)]
    fun test_calc_optimal_coin_values_4() {
        // 100 BTC, 28000 USDT
        let (_, _) = register_pool_with_liquidity(10000000000, 28000000000);

        // 1 BTC, 10000 USDT
        let x_desired = 100000000;
        let y_desired = 10000000000;

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"BTC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        let (_, _) =
            router::calc_optimal_coin_values<BTC, USDT, Uncorrelated>(
                x_desired,
                y_desired,
                0,
                2800000000,
                fa_x_metadata,
                fa_y_metadata,
            );
    }

    #[test]
    fun test_fee_config_for_uncorrelated_curve() {
        let (_, _) = register_pool_with_liquidity(10000, 10000);

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"BTC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        let (fee, d) = router::get_fees_config<BTC, USDT, Uncorrelated>(fa_x_metadata, fa_y_metadata);
        assert!(fee == 30, 1);
        assert!(d == 10000, 2);
        let (fee, d) = router::get_fees_config<USDT, BTC, Uncorrelated>(fa_y_metadata, fa_x_metadata);
        assert!(fee == 30, 3);
        assert!(d == 10000, 4);

        let fee = router::get_fee<BTC, USDT, Uncorrelated>(fa_x_metadata, fa_y_metadata);
        assert!(fee == 30, 5);
        let fee = router::get_fee<USDT, BTC, Uncorrelated>(fa_y_metadata, fa_x_metadata);
        assert!(fee == 30, 6);

        let (dao_fee, d) =
            router::get_dao_fees_config<BTC, USDT, Uncorrelated>(fa_x_metadata, fa_y_metadata);
        assert!(dao_fee == 33, 7);
        assert!(d == 100, 8);
        let (dao_fee, d) =
            router::get_dao_fees_config<USDT, BTC, Uncorrelated>(fa_y_metadata, fa_x_metadata);
        assert!(dao_fee == 33, 9);
        assert!(d == 100, 10);

        let dao_fee = router::get_dao_fee<BTC, USDT, Uncorrelated>(fa_x_metadata, fa_y_metadata);
        assert!(dao_fee == 33, 11);
        let dao_fee = router::get_dao_fee<USDT, BTC, Uncorrelated>(fa_y_metadata, fa_x_metadata);
        assert!(dao_fee == 33, 12);
    }

    #[test]
    fun test_fee_config_for_stable_curve() {
        let (_, _) = register_stable_pool_with_liquidity(10000, 10000);

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"USDC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        let (fee, d) = router::get_fees_config<USDC, USDT, Stable>(fa_x_metadata, fa_y_metadata);
        assert!(fee == 4, 1);
        assert!(d == 10000, 2);
        let (fee, d) = router::get_fees_config<USDT, USDC, Stable>(fa_y_metadata, fa_x_metadata);
        assert!(fee == 4, 3);
        assert!(d == 10000, 4);

        let fee = router::get_fee<USDC, USDT, Stable>(fa_x_metadata, fa_y_metadata);
        assert!(fee == 4, 5);
        let fee = router::get_fee<USDT, USDC, Stable>(fa_y_metadata, fa_x_metadata);
        assert!(fee == 4, 6);

        let (dao_fee, d) = router::get_dao_fees_config<USDC, USDT, Stable>(fa_x_metadata, fa_y_metadata);
        assert!(dao_fee == 33, 7);
        assert!(d == 100, 8);
        let (dao_fee, d) = router::get_dao_fees_config<USDT, USDC, Stable>(fa_y_metadata, fa_x_metadata);
        assert!(dao_fee == 33, 9);
        assert!(d == 100, 10);

        let dao_fee = router::get_dao_fee<USDC, USDT, Stable>(fa_x_metadata, fa_y_metadata);
        assert!(dao_fee == 33, 11);
        let dao_fee = router::get_dao_fee<USDT, USDC, Stable>(fa_y_metadata, fa_x_metadata);
        assert!(dao_fee == 33, 12);
    }

    #[test]
    #[expected_failure(abort_code = liquidity_pool::ERR_POOL_DOES_NOT_EXIST)]
    fun test_get_fees_config_fail_if_pool_does_not_exists() {
        let _ = test_coins::create_admin_with_fas();

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"BTC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        router::get_fees_config<BTC, USDT, Uncorrelated>(fa_x_metadata, fa_y_metadata);
    }

    #[test]
    #[expected_failure(abort_code = liquidity_pool::ERR_POOL_DOES_NOT_EXIST)]
    fun test_get_fee_fail_if_pool_does_not_exists() {
        let _ = test_coins::create_admin_with_fas();

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"BTC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        router::get_fee<BTC, USDT, Uncorrelated>(fa_x_metadata, fa_y_metadata);
    }

    #[test]
    #[expected_failure(abort_code = liquidity_pool::ERR_POOL_DOES_NOT_EXIST)]
    fun test_get_dao_fees_config_fail_if_pool_does_not_exists() {
        let _ = test_coins::create_admin_with_fas();

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"BTC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        router::get_dao_fees_config<BTC, USDT, Uncorrelated>(fa_x_metadata, fa_y_metadata);
    }

    #[test]
    #[expected_failure(abort_code = liquidity_pool::ERR_POOL_DOES_NOT_EXIST)]
    fun test_get_dao_fee_fail_if_pool_does_not_exists() {
        let _ = test_coins::create_admin_with_fas();

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"BTC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        router::get_dao_fee<BTC, USDT, Uncorrelated>(fa_x_metadata, fa_y_metadata);
    }

    #[test]
    fun test_get_amount_out_does_not_overflow_on_liquidity_close_to_u64_max() {
        // 100 BTC, 28000 USDT
        let (_, _) = register_pool_with_liquidity(2658758714820000, 28000000000);

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"BTC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        let _ = router::get_amount_out<BTC, USDT, Uncorrelated>(1, fa_x_metadata, fa_y_metadata);
    }

    #[test]
    fun test_get_amount_out_does_not_overflow_on_coin_in_close_to_u64_max() {
        // 100 BTC, 28000 USDT
        let (_, _) = register_pool_with_liquidity(2658758, 280000);

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"BTC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        let _ =
            router::get_amount_out<BTC, USDT, Uncorrelated>(2658758714820000, fa_x_metadata, fa_y_metadata);
    }

    #[test]
    fun test_get_amount_in_does_not_overflow_on_liquidity_x_close_to_u64_max() {
        // 100 BTC, 28000 USDT
        let (_, _) = register_pool_with_liquidity(2658758714820000, 28000000000);

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"BTC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        let _ = router::get_amount_in<BTC, USDT, Uncorrelated>(1, fa_x_metadata, fa_y_metadata);
    }

    #[test]
    fun test_get_amount_in_does_not_overflow_on_liquidity_y_close_to_u64_max() {
        // 100 BTC, 28000 USDT
        let (_, _) = register_pool_with_liquidity(28000000000, 2658758714820000);

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"BTC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        let _ = router::get_amount_in<BTC, USDT, Uncorrelated>(1, fa_x_metadata, fa_y_metadata);
    }

    #[test]
    #[expected_failure(abort_code = router::ERR_INSUFFICIENT_Y_AMOUNT)]
    fun test_get_amount_in_aborts_if_coin_out_bigger_than_reserves() {
        // 100 BTC, 28000 USDT
        let (_, _) = register_pool_with_liquidity(28000000000, 28000000000);

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"BTC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        let _ =
            router::get_amount_in<BTC, USDT, Uncorrelated>(2658758714820000, fa_x_metadata, fa_y_metadata);
    }

    #[test]
    #[expected_failure(abort_code = router::ERR_INSUFFICIENT_Y_AMOUNT)]
    fun test_get_amount_in_if_coin_out_exactly_equals_reserve_out() {
        // 100 BTC, 28000 USDT
        let (_, _) = register_pool_with_liquidity(28000000000, 28000000000);

        let fa_x_metadata = test_coins::get_fa_metadata_from_symbol(b"BTC");
        let fa_y_metadata = test_coins::get_fa_metadata_from_symbol(b"USDT");

        let _ = router::get_amount_in<BTC, USDT, Uncorrelated>(28000000000, fa_x_metadata, fa_y_metadata);
    }

    #[test]
    #[expected_failure(abort_code = router::ERR_FA_CONVERSION_OVERFLOW)]
    fun test_add_liquidity_with_imbalanced_reserves() {
        // 100 BTC, 28000 USDT
        let (fa_admin, lp_owner) = register_pool_with_liquidity(1, 10000000);

        let btc_in = test_coins::mint_fa(&fa_admin,  b"BTC", 2658758714820000);
        let usdt_in = test_coins::mint_fa(&fa_admin,  b"USDT", 1000);
        let (btc_out, usdt_out, lp_out) =
            router::add_liquidity<BTC, USDT, Uncorrelated>(btc_in, 1, usdt_in, 1);

        let lp_owner_addr = signer::address_of(&lp_owner);
        primary_fungible_store::deposit(lp_owner_addr, btc_out);
        primary_fungible_store::deposit(lp_owner_addr, usdt_out);
        primary_fungible_store::deposit(lp_owner_addr, lp_out);
    }
}
